...
function vc_rsfilter_mapping() {
    ...
    $args = array(
        'taxonomy' => array('category'),//ur taxonomy
        'hide_empty' => true,                  
    );
	...
	vc_map( 
		array(
			'name' => __('Rs Filter Post Module', 'rsaddon'),
			'base' => 'vc_rsfil',
			'description' => __('Rs Filter Post Module', 'rsaddon'), 
			...
			array(
					'type' => 'textfield',
					'holder' => 'h3',						
					'heading' => __( 'Default Title', 'rsaddon' ),
					'param_name' => 'default_title',
					'value' => __( '', 'rsaddon' ),
					'description' => __( 'You can add here title' ),
					'admin_label' => false,
					'weight' => 0,
					'value' => 'Featured News',
			...
			array(
				"type" => "dropdown",
				"heading" => __("Post Order", "rs-addon"),
 ...
 add_action( 'vc_before_init', 'vc_rsfilter_mapping' );
     
    // Element HTML
   function vc_rsriltrt_html( $atts,$content ) {
         $attributes = array();
        // Params extraction
        extract(
            shortcode_atts(
	   ...
	   //custom class added
		...
		$html='<div class="rs-filter-posts '.$css_class.' '.$css_class_custom.'">';

				if($show_filter !='No'):
				    $html .= '<div class="portfolio-filters  filter-'.strtolower($filter_align).'">
				    		<div class="default-title">'.$default_title.'</div>
				                <div class="default-btn"><button class="active" data-filter="*">'.$all_project.'</button>'; 			                
				...
				$html .= '<ul id="rs-filter-post" class="row grids">';				
				...
				while($best_wp->have_posts()): $best_wp->the_post();
					...
					$post_img_url = get_the_post_thumbnail_url($best_wp->ID,'khelo_blog_filter_grid');

					$post_content = '';
					if(function_exists('khelo_custom_excerpt')) {
						$post_content = khelo_custom_excerpt( '20' ); 
					}
					...
					$post_url = get_post_permalink($best_wp->ID); 
				{
					...
					$html .='<li class="col-lg-'.$col_group.' '.$termsString.'  grid-item gallery-space"> <div class="galley-img">';

						$html .='<div class="inner-content">
							<div class="post-img"><a href="'.$post_url.'"><img src="'.$post_img_url.'" alt="'.$post_title.'"></a></div>
								<div class="c-txt">
									<span class="category">'.$term->slug.'</span>
									<span class="post-dates"><i class="fa fa-calendar-check-o"></i>'.$post_date.'</span>
									<span class="post-author"><i class="glyph-icon flaticon-user-1"></i>'.$post_admin.'</span>
									<a href="'.$post_url.'"><h3 class="p-title">'.$post_title.'</h3></a>
									<p>	'.$post_content.'</p>
								</div>
							</div>
						</div>';
...
add_shortcode( 'vc_rsfil', 'vc_rsriltrt_html' );